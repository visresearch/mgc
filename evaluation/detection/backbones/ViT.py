...
class ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768,
                 ...
                 out_indices=[2, 5, 8, 11], abs = True, pretrained = None):
        """
        Args:
    ...
    def resize_pos_embed(self, pos_embed_old):
        ...
        w0 = self.img_size // self.patch_embed.patch_size
        h0 = self.img_size // self.patch_embed.patch_size
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.blocks):
            ...
            if i in self.out_indices:
                out = self.norm(x[:, 1:, :]).permute(0, 2, 1).reshape(B, -1, H, W).contiguous()
                outs.append(out)
